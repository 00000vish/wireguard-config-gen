...
export async function generateConfigs(provider, userPrivateKey, address, dns, useHostname) {
    let providerName = provider.name

    let rootDir = "./temp";
    let dirPath = `${rootDir}/${providerName}`
    ...
    for (let server of servers) {
        ...
        if (useHostname) {
            serverEndpoints = `${server.hostname}:${port}`;
        } else {
        ...
        let fileContent = getWireGuardConfig(userPrivateKey, address, dns, serverPublicKey, serverEndpoints);

        await writeToFile(filePath, fileContent);
    }

    let zipFilePath = `${rootDir}/${providerName}.zip`;
...
function getWireGuardConfig(userPrivateKey, address, dns, serverPublicKey, serverEndpoints) {
    let config = `# Generated using https://github.com/00000vish/wireguard-config-gen\n\n`

    if (userPrivateKey || address || dns) {
        let interfaceConfig = `[Interface]\n`;
        interfaceConfig += userPrivateKey.length === 0 ? "" : `PrivateKey = ${userPrivateKey.trim()}\n`;
        interfaceConfig += address.length === 0 ? "" : `Address = ${address.trim()}\n`;
        interfaceConfig += dns.length === 0 ? "" : `DNS = ${dns.trim()}\n`;
